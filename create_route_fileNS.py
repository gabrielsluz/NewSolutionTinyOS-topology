...
def get_routes(inputFile, outputFile):
    ...
    matches = re.findall(r"x(\d)0\[(\d+),(\d+)\] *\* *1", text)
    links0 = to_dict(matches)
    source = get_source(links)
    destination = get_destination(links)
    source_routes, relay_routes = add_channel(links, source, destination)
    source_routes0, relay_routes0 = add_channel(links0, source, destination)
    paths_len = len(source_routes) + len(relay_routes) + len(source_routes0) + len(relay_routes0)
    num_paths = len(source_routes) + len(source_routes0)
    ...
    outputText += "//uint8_t numHops = " + str(len(source_routes) + len(relay_routes)) + ";\n"
    ...
    for link in relay_routes:
        ...
        outputText +=           str(link["radio"]) + ", "
        outputText +=           str(link["channel"]) + "},\n"
    for link in source_routes0:
        outputText += "    {" + str(link["source"]) + ", "
        outputText +=           str(link["destination"]) + ", "
        outputText +=           str(link["radio"]) + ", "
        outputText +=           str(link["channel"]) + "},\n"
    for link in relay_routes0:
        outputText += "    {" + str(link["source"]) + ", "
        outputText +=           str(link["destination"]) + ", "
        outputText +=           str(link["radio"]) + ", "
        outputText +=           str(link["channel"]) + "},\n"
    ...
    print(paths_len,source,destination)
