...
//uint8_t numHops = 1;
  ...
  // End route
	...
	uint8_t realdestination = 0; 
	uint8_t realsource = 0; 
  ...
  void initializeNode() {
    ...
    uint8_t radio;
    uint8_t channel;
	realdestination = destinationNode;
	realsource = sourceNode;
  ...
  void sendMessage() {
	...
	if(TOS_NODE_ID == realdestination){
		sourceNode = realdestination;
		destinationNode = realsource;
	}
   ...
   // endTime = call FinishTimer.getNow();
	...
	//Sinaliza qual o no destino
	call SerialLogger.log(LOG_DEST_NODE,destinationNode);
    result = call MpdrSend.send(destinationNode, msg, sizeof(mpdr_test_msg_t));
    if (result == SUCCESS) {
      sendCount++;
    }
	sourceNode = realsource;
	destinationNode = realdestination;
  }
  ...
  event message_t* MpdrReceive.receive(message_t* msg, void* payload,
    ...
    endTime = call FinishTimer.getNow();
	call SerialLogger.log(LOG_COUNT_PARCIAL,receivedCount);
    //Nova parte
	if(TOS_NODE_ID == realdestinationNode){
	call MpdrControl.start();
	//Atualiza as tabelas e troca source com destination
	 ...
	 for (i = 0; i < numHops; i++) {
      ...
      if (TOS_NODE_ID == node && TOS_NODE_ID == realdestinationNode) {
        call MpdrRouting.addSendRoute(realdestinationNode ,realsourceNode, 
                                      next_hop, radio, channel);
      }
      if (TOS_NODE_ID == node && TOS_NODE_ID != realsourceNode) {
        call MpdrRouting.addRoutingItem(realdestinationNode, realsourceNode,
                                        next_hop, radio, channel);
      }
    }
	//Envia a msg de retorno
	sendMessage();
   
	}else if(TOS_NODE_ID == realsourceNode){
